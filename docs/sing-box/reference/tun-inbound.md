...
Bebra VPN генерирует имя с префиксом `xftun` и случайным суффиксом (например `xftun3a1b2c`).
...
**Предупреждение (Windows):** значение `true` может нарушить работу VirtualBox и вызвать петлю маршрутизации для direct-исходящих соединений на некоторых сборках Windows. Bebra VPN по умолчанию использует `false`.
...
Конфигурация TUN, генерируемая Bebra VPN по умолчанию:
